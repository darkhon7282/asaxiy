        ...
        
        {
          ...
          <div  key={inx} className="pro__items">
            <div className='product__data' to={"/"}>
              ...
              <p className='product__credit'>{Math.floor((item.price + (item.price * 0.3)) / 12)} So'm / 12 oy</p>
              {
                admin ?
                <button onClick={()=> deleteProduct(item.id)}>Delete</button>
                :
                <button onClick={()=> addToCart(item)}>Bir bosishda olish</button>
            ...
            <div className="product__reaction">
              <div  className='reaction_icon'>
                <BsCartFill onClick={()=> addToCart(item)} className='reaction_icon reaction_cart'/>
                {
                  likes?.some(i => i.id === item.id) ? 
                  <AiFillHeart style={{color: "#008DFF"}} onClick={()=> dispatch({type:REMOVE_LIKE, payload: item.id})}/> : 
