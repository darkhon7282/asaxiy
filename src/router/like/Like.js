...
import "./Like.css";
import empty from "../../assets/hearts.avif";
...
function Like() {
  ...
  const addToCart = (item) => {
    ...
    if (index < 0) {
      return dispatch({
        type: ADD_TO_CART,
        payload: [...cart, { ...item, qty: 1 }],
      });
    }
    let newCart = cart.map((pro, inx) =>
      ...
      {like.length === 0 ? (
        ...
        <div className="">
          ...
          <div className="like__full">
            {like?.map((item, inx) => (
              <div key={inx} className="favourites">
                <div className="like__product">
                  ...
                  <div className="like__download">
                    ...
                    <button
                      onClick={() =>
                        dispatch({ type: REMOVE_LIKE, payload: item.id })
                      }
                      className="like_btn2"
                    >
                      O'chirish
